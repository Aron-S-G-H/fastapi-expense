...
from src.schemas.auth_schema import LoginSchema, RegisterSchema, RegisterResponseSchema, RefreshTokenSchema
...
def get_current_user(db: db_dependency, credentials: credential_dependency, _: locale_dependency) -> UserModel:
    token = credentials.credentials
    try:
        payload = jwt.decode(token, key=settings.SECRET_KEY, algorithms=settings.ALGORITHM)
        token_type = payload.get('type', None)
        if token_type != 'access':
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=_('token type is invalid'))
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=f'decode error: {e}')
    except InvalidSignatureError as e:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=f'signature error: {e}')
...
def decode_refresh_token(token) -> int:
    try:
        ...
        user_id = payload.get('user_id', None)
        if not user_id:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='token is not correct')
        now = get_current_time()
        token_exp = payload.get('exp', None)
        if now > datetime.fromtimestamp(token_exp, tz=timezone.utc):
