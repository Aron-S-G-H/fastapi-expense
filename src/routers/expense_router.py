...
from src.dependencies import db_dependency, user_dependency, locale_dependency
...
def get_expense(db: db_dependency, user: user_dependency, _: locale_dependency, expense_id: int = Path(..., gt=0)):
    expense = db.query(ExpenseModel).filter_by(id=expense_id, user_id=user.id).one_or_none()
    if expense:
        return expense
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=_("Expense not found"))
...
def update_expense(db: db_dependency, user: user_dependency, _: locale_dependency, expense_id: int = Path(..., gt=0), data: UpdateExpenseSchema = Body(...)):
    expense = db.query(ExpenseModel).filter_by(id=expense_id, user_id=user.id).one_or_none()
    if expense:
...
def delete_expense(db: db_dependency, user: user_dependency, _: locale_dependency, expense_id: int = Path(..., gt=0)):
    expense = db.query(ExpenseModel).filter_by(id=expense_id, user_id=user.id).one_or_none()
    if expense:
        db.delete(expense)
        db.commit()
        return JSONResponse({"detail": _("Expense deleted")})
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=_("Expense not found"))